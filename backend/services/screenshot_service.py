...
# Singleton browser instance (only for non-session sources)
...
async def _ensure_browser():
    """Get or create the persistent browser singleton."""
...
async def take_screenshot(url: str, source_name: str, source_data: dict = None) -> bytes:
    ...
    # ── Non-session sources (keep current headless approach) ─────────────────
    browser = await _ensure_browser()

    # Check if a saved session exists for this source (legacy path, won't trigger
    # for non-session sources, but kept for compatibility)
    storage_state = None
    if source_key in SESSION_MAP:
        session_path = SESSION_MAP[source_key]
        if os.path.exists(session_path):
            ...
            print(f"[SESSION] Using saved session for {source_name}")
    ...
    # ThreatBook-specific stealth measures (reCAPTCHA detection bypass)
    if source_key == "threatbook":
        await context.add_init_script("""
...
async def close_browser():
    """Clean shutdown of the browser singleton."""
    global _browser, _playwright
    if _browser:
